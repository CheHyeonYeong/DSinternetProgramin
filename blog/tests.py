...
class TestView(TestCase):
    def setUp(self):
        ...
        self.tag_python_kor = Tag.objects.create(name='python_study', slug='python-study')
    ...
    def test_create_post(self):
        respons = self.client.get("/blog/create_post/")
        self.assertNotEqual(respons.status_code, 200)

        self.client.login(username='moha', password="somepassword")
        respons = self.client.get("/blog/create_post/")
        # 정상적으로 페이지가 로드되는가
        ...
        self.assertEqual(last_post.author.username, 'moha')
